...
@interface _BLModelPropertyMeta : NSObject {
    @package
    ...
    _BLModelPropertyMeta *_next; ///< next meta if there are multiple properties mapped to the same key.
}
@end

@implementation _BLModelPropertyMeta
+ (instancetype)metaWithClassInfo:(BLClassInformation *)classInfo propertyInfo:(YYClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    _BLModelPropertyMeta *meta = [self new];
...
@interface _BLModelMeta : NSObject {
    @package
    BLClassInformation *_classInfo;
    /// Key:mapped key and key path, Value:_BLModelPropertyMeta.
    NSDictionary *_mapper;
    /// Array<_BLModelPropertyMeta>, all property meta of this model.
    NSArray *_allPropertyMetas;
    /// Array<_BLModelPropertyMeta>, property meta which is mapped to a key path.
    NSArray *_keyPathPropertyMetas;
    /// Array<_BLModelPropertyMeta>, property meta which is mapped to multi keys.
...
@implementation _BLModelMeta
- (instancetype)initWithClass:(Class)cls {
    BLClassInformation *classInfo = [BLClassInformation classInfoWithClass:cls];
    ...
    if ([cls respondsToSelector:@selector(modelPropertyBlacklist)]) {
        NSArray *properties = [(id<BLModel>)cls modelPropertyBlacklist];
        if (properties) {
    ...
    if ([cls respondsToSelector:@selector(modelPropertyWhitelist)]) {
        NSArray *properties = [(id<BLModel>)cls modelPropertyWhitelist];
        if (properties) {
    ...
    if ([cls respondsToSelector:@selector(modelContainerPropertyGenericClass)]) {
        genericMapper = [(id<BLModel>)cls modelContainerPropertyGenericClass];
        if (genericMapper) {
    ...
    BLClassInformation *curClassInfo = classInfo;
    while (curClassInfo && curClassInfo.superCls != nil) { // recursive parse super class, but ignore root class (NSObject/NSProxy)
        for (YYClassPropertyInfo *propertyInfo in curClassInfo.propertyInfos.allValues) {
            if (!propertyInfo.name) continue;
            if (blacklist && [blacklist containsObject:propertyInfo.name]) continue;
            if (whitelist && ![whitelist containsObject:propertyInfo.name]) continue;
            _BLModelPropertyMeta *meta = [_BLModelPropertyMeta metaWithClassInfo:classInfo
    ...
    if ([cls respondsToSelector:@selector(modelCustomPropertyMapper)]) {
        NSDictionary *customMapper = [(id <BLModel>)cls modelCustomPropertyMapper];
        [customMapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *mappedToKey, BOOL *stop) {
            _BLModelPropertyMeta *propertyMeta = allPropertyMetas[propertyName];
    ...
    [allPropertyMetas enumerateKeysAndObjectsUsingBlock:^(NSString *name, _BLModelPropertyMeta *propertyMeta, BOOL *stop) {
...
+ (instancetype)metaWithClass:(Class)cls {
    ...
    _BLModelMeta *meta = CFDictionaryGetValue(cache, (__bridge const void *)(cls));
    dispatch_semaphore_signal(lock);
    if (!meta || meta->_classInfo.needUpdate) {
        meta = [[_BLModelMeta alloc] initWithClass:cls];
        if (meta) {
            dispatch_semaphore_wait(lock, DISPATCH_TIME_FOREVER);
...
static force_inline NSNumber *ModelCreateNumberFromProperty(__unsafe_unretained id model,
                                                            __unsafe_unretained _BLModelPropertyMeta *meta) {
    switch (meta->_type & YYEncodingTypeMask) {
        case YYEncodingTypeBool: {
...
static void ModelSetWithDictionaryFunction(const void *_key, const void *_value, void *_context) {
    ...
    __unsafe_unretained _BLModelMeta *meta = (__bridge _BLModelMeta *)(context->modelMeta);
    __unsafe_unretained _BLModelPropertyMeta *propertyMeta = [meta->_mapper objectForKey:(__bridge id)(_key)];
...
/**
 ...
 @param _propertyMeta should not be nil, _BLModelPropertyMeta.
 @param _context      _context.model and _context.dictionary should not be nil.
 */
static void ModelSetWithPropertyMetaArrayFunction(const void *_propertyMeta, void *_context) {
    ModelSetContext *context = _context;
    __unsafe_unretained NSDictionary *dictionary = (__bridge NSDictionary *)(context->dictionary);
    __unsafe_unretained _BLModelPropertyMeta *propertyMeta = (__bridge _BLModelPropertyMeta *)(_propertyMeta);
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    _BLModelMeta *modelMeta = [_BLModelMeta metaWithClass:[model class]];
    if (!modelMeta || modelMeta->_keyMappedCount == 0) return nil;
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:64];
    __unsafe_unretained NSMutableDictionary *dic = result; // avoid retain and release in block
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _BLModelPropertyMeta *propertyMeta, BOOL *stop) {
    ...
    if (modelMeta->_hasCustomTransformToDictionary) {
        BOOL suc = [((id<BLModel>)model) modelCustomTransformToDictionary:dic];
        if (!suc) return nil;
    }
    return result;
}
    ...
    switch (modelMeta->_nsType) {
        ...
        default: {
            ...
            NSArray *properties = [modelMeta->_allPropertyMetas
                                   sortedArrayUsingComparator:^NSComparisonResult(_BLModelPropertyMeta *p1, _BLModelPropertyMeta *p2) {
                                       return [p1->_name compare:p2->_name];
                                   }];
            
            [desc appendFormat:@" {\n"];
            for (NSUInteger i = 0, max = properties.count; i < max; i++) {
                _BLModelPropertyMeta *property = properties[i];
...
@implementation NSObject (BLModel)
...
+ (instancetype)modelWithDictionary:(NSDictionary *)dictionary {
    ...
    _BLModelMeta *modelMeta = [_BLModelMeta metaWithClass:cls];
...
- (BOOL)modelSetWithDictionary:(NSDictionary *)dic {
    ...
    _BLModelMeta *modelMeta = [_BLModelMeta metaWithClass:object_getClass(self)];
    if (modelMeta->_keyMappedCount == 0) return NO;
    
    if (modelMeta->_hasCustomWillTransformFromDictionary) {
        dic = [((id<BLModel>)self) modelCustomWillTransformFromDictionary:dic];
        if (![dic isKindOfClass:[NSDictionary class]]) return NO;
    }
    ...
    if (modelMeta->_hasCustomTransformFromDictionary) {
        return [((id<BLModel>)self) modelCustomTransformFromDictionary:dic];
    }
    return YES;
}
...
- (id)modelCopy{
    ...
    _BLModelMeta *modelMeta = [_BLModelMeta metaWithClass:self.class];
    if (modelMeta->_nsType) return [self copy];
    
    NSObject *one = [self.class new];
    for (_BLModelPropertyMeta *propertyMeta in modelMeta->_allPropertyMetas) {
...
@implementation NSArray (BLModel)
...
@implementation NSDictionary (BLModel)
